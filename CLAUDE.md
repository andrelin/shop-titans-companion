...
**Data pipeline (build-time, not runtime).** `scripts/sync-data.ts` downloads tabs from the official Shop Titans Google Sheet (maintained by Kabam, short link `playshoptitans.com/spreadsheet`) as CSV, parses them by fixed column index, and writes typed JSON to `data/` — which is committed to git. The app loads that committed JSON via a static import (`src/data/load.ts`), so there is no runtime fetch of game data. A scheduled GitHub Action (`.github/workflows/sync-data.yml`, daily 06:00 UTC) re-runs the sync and commits any diff. A separate Action (`pages.yml`) builds and deploys on push to `main`.
...
- Each item has two enchant slots (element + spirit). Element/spirit enchants apply at the **player's** unlocked tier (`maxEnchantTier`), not the item's tier. The per-tier stat additions are derived at module load from the synced enchant blueprints (`data/enchantments.json`) into `ELEMENT_TABLE` and `SPIRIT_TABLE` — **elements and spirits have different base stats below T9**, and the element slot reads the element table while the spirit slot reads the spirit table (or a specific family's stats, for event-buffed spirits like Tiger/Christmas/Krampus/Kirin). The affinity-match value is **`floor(1.5 × base)`**. Don't hand-edit these tables — fix the data/sync if a value is wrong.
...
- **The goal is to reproduce the in-game AP exactly, not "approximately."** The model matches nearly every verified reading to the exact integer (see the live count in `docs/data-points.md`); the remaining gaps are the next thing to fix, not slack to live with. Don't introduce changes that widen any existing exact match into a tolerance.
...
- **Never fabricate item data — affinities, bonuses, stats, built-in enchants — to make a test pass.** A test's blueprint must reflect the item's real synced data (`data/blueprints.json`). If the model and an in-game reading disagree, the bug is in the *model*, not the data — fix the formula, never invent an affinity or bonus the item doesn't have. (This is exactly how the Lone Wolf Cowl starforge bug hid for so long: the test gave LWC a phantom "Dark" affinity so a wrong base-only boost happened to land on the right number. Two compensating errors looked green until a genuinely no-affinity item exposed both.) When a test needs an affinity the data lacks but the game clearly has, that's a *data gap to investigate and fix at the source* (sheet/sync), recorded in the docs — not something to hardcode into a test.
...
- **Keep user-facing data correct, not just the model.** Any change to the power model, stat values, rounding, or mechanics means the in-app "How airship power is calculated" explainer (`src/tools/DragonInvasion/DragonInvasion.tsx`) and any per-item displays (e.g. the bonus badges in the results table) can go stale and mislead users. After such a change, *always* re-read the explainer and the affected UI and confirm every statement still matches the code — update both in the same change. A user reading a stale explanation is worse than no explanation. Treat user-facing copy as part of the calibration surface: if it claims a behaviour the model no longer has, that's a bug.

**`docs/data-points.md`** is the calibration log: a table of in-game AP readings the model is verified against, what each reading nailed down, the open unresolved cases, and a to-craft priority list of useful future readings. When the user reports a new in-game number, this is where it gets recorded *and* a corresponding case is added to `enchant.test.ts`. Enchant tiers and base stats are no longer guessed — they're synced from each enchant's own blueprint row (Type = "Enchantment") in the Blueprints tab, so they're authoritative; the match value is `floor(1.5 × base)`.
